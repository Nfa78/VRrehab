...
namespace UnityEditor.XR.Interaction.Toolkit.Samples.Hands.Editor
{
    ...
    static class HandsSampleProjectValidation
    {
        ...
        static List<BuildValidationRule> s_BuildValidationRules;
        ...
        static void RegisterProjectValidationRules()
        {
            ...
            {
                BuildValidator.AddRules(buildTargetGroup, BuildValidationRules);
            }
        ...
        {
            foreach (var validation in BuildValidationRules)
            {
        ...
        static List<BuildValidationRule> BuildValidationRules
        {
            get
            {
                if (s_BuildValidationRules != null)
                    return s_BuildValidationRules;

                var handsVersion = SafeGetPackageVersion(k_HandsPackageName);
                var xriVersion = SafeGetPackageVersion(k_XRIPackageName);

                s_BuildValidationRules = new List<BuildValidationRule>
                {
                    new BuildValidationRule
                    {
                        ...
                        Category = k_Category,
                        CheckPredicate = () => SafeGetPackageVersion(k_HandsPackageName) >= s_MinimumPackageVersion,
                        FixIt = () =>
                        {
                    ...
                    new BuildValidationRule
                    {
                        ...
                        Category = k_Category,
                        CheckPredicate = () => SafeGetPackageVersion(k_HandsPackageName) >= s_RecommendedPackageVersion,
                        FixIt = () =>
                        {
                            if (s_HandsPackageAddRequest == null || s_HandsPackageAddRequest.IsCompleted)
                                InstallOrUpdateHands();
                    ...
                    new BuildValidationRule
                    {
                        IsRuleEnabled = () => handsVersion >= s_MinimumPackageVersion,
                        ...
                        Category = k_Category,
                        CheckPredicate = () => ProjectValidationUtility.SampleImportMeetsMinimumVersion(k_HandsPackageDisplayName, k_HandVisualizerSampleName, SafeGetPackageVersion(k_HandsPackageName)),
                        FixIt = () =>
                        {
                    ...
                    new BuildValidationRule
                    {
                        Message = $"[{k_SampleDisplayName}] {k_StarterAssetsSampleName} sample from XR Interaction Toolkit ({k_XRIPackageName}) package must be imported or updated to use this sample. {GetImportSampleVersionMessage(k_Category, k_StarterAssetsSampleName, xriVersion)}",
                        Category = k_Category,
                        CheckPredicate = () => ProjectValidationUtility.SampleImportMeetsMinimumVersion(k_Category, k_StarterAssetsSampleName, SafeGetPackageVersion(k_XRIPackageName)),
                        FixIt = () =>
                        {
        ...
        static PackageVersion SafeGetPackageVersion(string packageName)
        {
            try
            {
                return PackageVersionUtility.GetPackageVersion(packageName);
            }
            catch
            {
                return default;
