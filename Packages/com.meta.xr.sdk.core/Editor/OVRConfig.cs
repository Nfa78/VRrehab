...
public class OVRConfig : ScriptableObject
{
    ...
    public static string GetAndroidSDKPathLocation(bool throwError = true)
    {
        ...
        if (useEmbedded)
        {
            ...
            {
                androidSDKPath =
                    Path.Combine(BuildPipeline.GetPlaybackEngineDirectory(BuildTarget.Android, BuildOptions.None), "SDK");
            }
            catch (Exception)
            {
                return string.Empty;
    ...
    public string GetGradlePath(bool throwError = true)
    {
        ...
        bool useEmbedded = EditorPrefs.GetBool("GradleUseEmbedded") ||
                           string.IsNullOrEmpty(EditorPrefs.GetString("GradlePath"));

        if (useEmbedded)
        {
            libPath = Path.Combine(BuildPipeline.GetPlaybackEngineDirectory(BuildTarget.Android, BuildOptions.None),
                "Tools\\gradle\\lib");
        }
        else
        {
...
#else
        libPath = Path.Combine(EditorApplication.applicationContentsPath,
            "PlaybackEngines\\AndroidPlayer\\Tools\\gradle\\lib");
#endif
    ...
    public string GetJDKPath(bool throwError = true)
    {
        ...
        bool useEmbedded = EditorPrefs.GetBool("JdkUseEmbedded") ||
                           string.IsNullOrEmpty(EditorPrefs.GetString("JdkPath"));

        string exePath = "";
        if (useEmbedded)
        {
#if UNITY_2019_1_OR_NEWER
            exePath = Path.Combine(BuildPipeline.GetPlaybackEngineDirectory(BuildTarget.Android, BuildOptions.None),
                "Tools\\OpenJDK\\Windows\\bin");
#else
            exePath = Path.Combine(EditorApplication.applicationContentsPath,
                "PlaybackEngines\\AndroidPlayer\\Tools\\OpenJDK\\Windows\\bin");
#endif
        }
        else
        {
